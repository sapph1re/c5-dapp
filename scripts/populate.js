  ...
  try {
    ...
    for (let i = 0; i < sampleData.data.length; i++) {
      ...
      await flightTickets.addAirline(
        web3.utils.asciiToHex(row.airline.aName),
        aOwner,
      ...
      for (let j = 0; j < row.tickets.length; j++) {
        ...
        await flightTickets.addTicket(
          aId,
          web3.utils.asciiToHex(ticket.tFrom),
          web3.utils.asciiToHex(ticket.tTo),
          web3.utils.toWei(ticket.tPrice.toString(), 'ether'),
          ticket.tQuantity,
          departure,
          arrival,
